...
make wpj428 || die "Failed to set build target"

make || die "Failure during u-boot build"

[ -e u-boot.bin ] || die "Build succeeded. But u-boot.bin wasn't created"

mkimage -A arm -O linux -C none -T kernel -e 0x84200000 -a 0x84200000 -n WPJ428-UBOOT -d u-boot.bin wpj428-uboot.bin

[ -e wpj428-uboot.bin ] || die "Failed to create wpj428-uboot.bin."
